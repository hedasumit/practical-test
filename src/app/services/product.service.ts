...
export class ProductService {
  ...
  updateProduct(product): void {
    ...
    this.products = this.localStorageGetItem('products');
    let objIndex = this.products.findIndex((obj => obj.id == product.id));
  ...
  deleteProduct(id): void {
    ...
    let newList = this.products.filter(obj => {
      return obj.id !== parseInt(id)
    });
    this.trashProducts = this.localStorageGetItem('trashProducts');
  ...
  restoreProduct(id) : void {
   ...
   this.trashProducts = this.localStorageGetItem('trashProducts');
    let result = this.trashProducts.filter(obj => {
      return obj.id === parseInt(id)
    });
    let newList = this.trashProducts.filter(obj => {
      return obj.id !== parseInt(id)
    });
    this.products = this.localStorageGetItem('products');
    if(this.products === null) {
  ...
  removeProduct(id) : void {
    this.trashProducts = this.localStorageGetItem('trashProducts');
    let objIndex = this.trashProducts.findIndex((obj => obj.id == id));
